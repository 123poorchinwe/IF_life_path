...
function corsHeaders(req: Request) {
  ...
  const configured = (process.env.CORS_ALLOWED_ORIGIN || "")
    ...
    .filter(Boolean);
  const accepted =
    [...allowedOrigins, ...configured].includes(origin) ||
    /^https:\/\/[a-z0-9-]+\.tcloudbaseapp\.com$/i.test(origin);
  return {
    "Access-Control-Allow-Origin": accepted ? origin : allowedOrigins[0],
...
function fallbackCards(text: string): ProfileCard[] {
  ...
  const skillMap: Array<[RegExp, string]> = [
    [/python/i, "Python"], [/sql/i, "SQL"], [/arcgis|qgis|gis/i, "GIS / 空间分析"],
    [/java/i, "Java"], [/javascript|typescript|react|vue/i, "Web 开发"],
    [/遥感|remote sensing/i, "遥感分析"], [/机器学习|machine learning/i, "机器学习"],
  ];
  skillMap.forEach(([pattern, label]) => {
    ...
    const response = await fetch(`${provider.baseUrl.replace(/\/$/, "")}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `请将以下材料解析为3到14张卡。类型只能是：事实、能力、推断、资格、限制、偏好。事实/能力/资格中明确来自原文的 confirmed=true；推断、证据不足和需要用户补充的内容 confirmed=false。输出 {"cards":[{"type":"事实","title":"...","detail":"...","evidence":"原文短语或证据说明","confirmed":true}]}。\n\n材料：\n${text.slice(0, 12000)}` },
        ],
      }),
    });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { cards: aiCards || fallbackCards(text), sourceText: text.slice(0, 20000), sourceName, usedAI: Boolean(aiCards) },
      { headers },
    );
  } catch (error) {
