...
type CareerData = {
  ...
  gaps: number;
  confidence: string;
  kind: "career" | "profile" | "skill" | "constraint";
};
...
const CareerNode = memo(function CareerNode({
  data,
  selected,
}: NodeProps<Node<CareerData>>) {
  return (
    <div
      className={`flow-node ${data.kind} ${statusKey[data.status]} ${selected ? "selected" : ""}`}
    >
      ...
      <div className="node-foot">
        ...
        {data.kind === "career" && (
          <small>{data.gaps ? `缺 ${data.gaps} 项条件` : "当前可尝试"}</small>
        )}
      </div>
...
function NetworkInner({ onEnter }: { onEnter: (missionId?: string) => void }) {
  ...
  const relevantOutcome = ["gis-dev", "institute", "phd"].includes(
    ...
    ? missionOutcomes.scope_and_credit
    : ["urban", "data-product", "ba"].includes(selectedBase.id)
      ? missionOutcomes.career_interview
      : undefined;
  const selected =
    relevantOutcome
      ? {
          ...selectedBase,
          status: "已开放" as Status,
          confidence: "任务证据支持",
          evidence: [
            ...selectedBase.evidence,
            `调查结局：${relevantOutcome.title}`,
          ],
        }
      : selectedBase;
    ...
    const visible = careers
      .filter(
        (c) =>
          ...
          (!excluded || c.status !== "硬性受限"),
      )
      .slice(0, 18);
    const ns: Node<CareerData>[] = [
      {
        id: "profile",
        type: "career",
        position: { x: 20, y: 305 },
      ...
      ...visible.map((c, i) => ({
        id: c.id,
        type: "career",
        position: { x: 610 + (i % 4) * 250, y: 45 + Math.floor(i / 4) * 145 },
        data: {
          label: c.title,
    ...
    const es: Edge[] = [
      {
        ...
        source: "profile",
        target: "skill-python",
        className: "edge-direct",
      },
      {
        id: "p-sp",
        source: "profile",
        target: "skill-spatial",
        className: "edge-direct",
      },
      {
        id: "p-co",
      ...
      <section className="network-workspace">
        <div className="network-toolbar">
          <div className="view-switch">
            <button
              className={view === "district" ? "active" : ""}
          ...
          {view === "network" && (
            <>
              ...
              <button onClick={() => setExcluded(!excluded)}>
                <Eye />
                {excluded ? "显示受限" : "隐藏受限"}
              </button>
        ...
        {view === "district" ? (
          ...
          <ReactFlow
            ...
            nodeTypes={nodeTypes}
            onNodeClick={(_, n) =>
              n.data.kind === "career" && setSelectedId(n.id)
            }
            fitView
            minZoom={0.35}
            maxZoom={1.5}
          ...
          >
            ...
            <MiniMap
              ...
              zoomable
              nodeColor={(n) =>
                n.data.kind === "career" ? "#526873" : "#304551"
              }
            />
          </ReactFlow>
        )}
        <div className="network-caption">
          ...
          {view === "district"
            ? "选择职业区域 · 建设证据设施 · 解锁新事件"
            : "拖动画布探索 · 滚轮缩放 · 点击职业查看证据"}
        </div>
      </section>
      <aside className="inspector">
        <button className="inspector-close" aria-label="关闭详情">
          <X />
        </button>
        ...
        <div className="inspector-actions">
          <Button onClick={() => onEnter()}>进入情景体验</Button>
          <Button variant="secondary">调查此路径</Button>
          <Button variant="ghost">暂时排除</Button>
        </div>
      </aside>
    </div>
  );
}
export default function CareerNetwork(props: {
